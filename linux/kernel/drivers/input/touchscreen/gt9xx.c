...
static const char *goodix_ts_name = "himax-touchscreen";
static struct workqueue_struct *goodix_wq;
struct i2c_client * i2c_connect_client = NULL; 
u8 config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
                = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};

//for large touch mask use
...
//add for factory
static u8 fac_mode=0; //0: out factory mode;1:in factory mode;

//add for glove mode
static u8 glove_on=0; //0 : out glove mode;1:in glove mode; 
static s32 gtp_init_panel(struct goodix_ts_data *ts);
//add for switch dev
struct switch_dev gt_touch_dev;
unsigned int gt_fw = 1050;
unsigned int cfg;
...
if((finger & 0x80) == 0)
    {
        if(large_touch_mask == 1)     //make sure key release
        {
                        large_touch_mask = 2;   //change to wait one key
        }
        goto exit_work_func;
    }
        ...
        if(large_touch_mask == 2)             //wait one key input
        {
            if(finger == 0x81)                        
            {   
                large_touch_mask = 0;  //if one key get, then clear large touch mask
            }
        }

        if(large_touch_mask != 0)              //if large touch mask 1 or 2 not zero then just return and exit.
        {
                goto exit_work_func;
        }
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
...
#if GTP_GLOVE_ON    
    //add for glove mode
    u8 cfg_info_group0[] = CTP_CFG_GROUP0;	//use glove config
#endif
    ...
    GTP_DEBUG_FUNC();
    GTP_DEBUG("Config Groups\' Lengths: %d, %d, %d, %d, %d, %d, %d", 
        cfg_info_len[0], cfg_info_len[1], cfg_info_len[2], cfg_info_len[3],
        cfg_info_len[4], cfg_info_len[5],cfg_info_len[6]);

    printk("[Red_debug] enter %s\n", __func__);
#if GTP_COMPATIBLE_MODE
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ts->fw_error = 0;
    ...
    else
    {
        ...
        if(glove_on == 1)
		{
			printk("[Red_debug]It's in glove mode,use group0\n");
			sensor_id = 0;
		}
		else
		{
			printk("[Red_debug]It's finger mode,Sensor_ID++\n");
			sensor_id++;
		}
#endif
...
/*******************************************************
add by red_zhang@asus.com for factory and glove
*******************************************************/
//================================================================
...
#if GTP_GLOVE_ON
/* Glove control by red_zhang@asus.com 20141110*/
static ssize_t gt_glove_set(struct device *dev,struct device_attribute *attr, char *buf)
{
	struct goodix_ts_data *ts = dev_get_drvdata(dev);
	int command;
	...
	if(command == 0)
		{
			glove_on = 0;
			gtp_init_panel(ts);
			return scnprintf(buf, PAGE_SIZE, "disable glove success!\n");
		}
	else if(command == 1)
	{
			glove_on = 1;
			gtp_init_panel(ts);
			return scnprintf(buf, PAGE_SIZE, "enable glove success!\n");	
	}
	else
	{
...
static ssize_t gt_glove_status(struct device *dev,struct device_attribute *attr, char *buf)
{
	if(glove_on == 0)
	{
		return scnprintf(buf, PAGE_SIZE, "glove_off\n");
	}
	else if(glove_on == 1)
	{
		return scnprintf(buf, PAGE_SIZE, "glove_on\n");
	}
	else return scnprintf(buf, PAGE_SIZE, "error number : %d\n", glove_on);
}
#endif
...
#if GTP_GLOVE_ON
static DEVICE_ATTR(glove_control, S_IRUGO|S_IWUSR, gt_glove_status, gt_glove_set);
#endif
...
//=================================================================
//add for switch dev
static ssize_t gt_touch_switch_name(struct switch_dev *sdev, char *buf)
{
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (ret < 0)
    {
        ...
        printk("<Red_debug> goodix chip doesn't exits\n");
        goto exit_probe;
    }
	...
	//add for switch dev
	gt_touch_dev.name = "touch";
	gt_touch_dev.print_name = gt_touch_switch_name;
	if (switch_dev_register(&gt_touch_dev) < 0) {
		GTP_ERROR("fail to register touch switch\n");
	}
...
exit_probe:
    ...
    GTP_INFO("GTP driver removing because chip doesn't exit\n");
...
static int goodix_ts_remove(struct i2c_client *client)
{
	...
	sysfs_remove_group(&client->dev.kobj, &gt_attr_group);
