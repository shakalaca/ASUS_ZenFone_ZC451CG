...
//	reportdata_z = rawdata_z;
    ...
    //<-- ASUS-Bevis_Chen + -->
		...
		//printk("%s : enAccelConfig_flag = : %d \n",__func__,enAccelConfig_flag);
...
{	
	...
	reportdata_x = 1024*(rawdata_y - tj9->accel_cal_offset[0])/tj9->accel_cal_sensitivity[0];
	reportdata_y = 1024*(-rawdata_x - tj9->accel_cal_offset[1])/tj9->accel_cal_sensitivity[1];
	reportdata_z = 1024*(rawdata_z - tj9->accel_cal_offset[2])/tj9->accel_cal_sensitivity[2];
			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	//}
		...
		/*-----------------------------------------------------------------*/
			temp_axis.x = reportdata_x;
			temp_axis.y = reportdata_y;
			temp_axis.z = reportdata_z;

			if (enAccelConfig_flag == 1)
			{
				temp_axis.x =enAccelConfig_func(temp_axis.x,  Max_x,  Min_x); //use calibration data without reboot	
				temp_axis.y =enAccelConfig_func(temp_axis.y,  Max_y,  Min_y);
				temp_axis.z =enAccelConfig_func(temp_axis.z,  Max_z,  Min_z);
			}
