...
struct proc_dir_entry *compass_entry = NULL;

static int compass_proc_show(struct seq_file *m, void *v) {
	if(!compass_entry)
...
static int compass_proc_open(struct inode *inode, struct  file *file) {
  return single_open(file, compass_proc_show, NULL);
}

static const struct file_operations compass_proc_fops = {
  .owner = THIS_MODULE,
  .open = compass_proc_open,
  .read = seq_read,
  .llseek = seq_lseek,
  .release = single_release,
};
int create_asusproc_compass_status_entry( void )
{   
  compass_entry = proc_create("asus_compass_a500_status", S_IWUGO| S_IRUGO, NULL,&compass_proc_fops);
    if (!compass_entry)
        return -ENOMEM;
...
//<-- ASUS-Bevis_chen + -->
	#ifdef CONFIG_ASUS_FACTORY_SENSOR_MODE
	if(create_asusproc_compass_status_entry( ))
		printk("[%s] : ERROR to create compass proc entry\n",__func__);
	#endif
//<-- ASUS-Bevis_chen - -->
	return 0;
...
static void __exit akm_compass_exit(void)
{
	...
	#ifdef CONFIG_ASUS_FACTORY_SENSOR_MODE
	if (compass_entry)
		remove_proc_entry("asus_compass_a500_status", NULL);
	#endif
	//<-- ASUS-Bevis_Chen - -->
