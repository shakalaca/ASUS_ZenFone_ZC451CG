...
//<-- ASUS-Bevis_chen+ --> for factory test  use
struct proc_dir_entry *hall_entry = NULL;

static int hallsensor_proc_show(struct seq_file *m, void *v) {
	if(!hall_sensor_dev)
...
static int hallsensor_proc_open(struct inode *inode, struct  file *file) {
  return single_open(file, hallsensor_proc_show, NULL);
}

static const struct file_operations hallsensor_proc_fops = {
  .owner = THIS_MODULE,
...
{   
    hall_entry = proc_create("asus_hallsensor_status", S_IWUGO| S_IRUGO, NULL,&hallsensor_proc_fops);
    if (!hall_entry)
        return -ENOMEM;
...
static void __exit hall_sensor_exit(void)
{
	...
	#ifdef CONFIG_ASUS_FACTORY_SENSOR_MODE
	if (hall_entry)
		remove_proc_entry("asus_hallsensor_status", NULL);
	#endif
	//<-- ASUS-Bevis_chen + -->
}
