...
extern char FactoryGGBXFile_ZC451[];
...
int get_voltage_now()
{
	int vol;
	mutex_lock(&ug31->info_update_lock);
	vol = ug31_module.get_voltage_now();
	mutex_unlock(&ug31->info_update_lock);
	return vol;
}
EXPORT_SYMBOL(get_voltage_now);
int get_current_now()
{
	int cur;
	mutex_lock(&ug31->info_update_lock);
	cur = ug31_module.get_current_now();
	mutex_unlock(&ug31->info_update_lock);
	return cur;
}
EXPORT_SYMBOL(get_current_now);
#endif
