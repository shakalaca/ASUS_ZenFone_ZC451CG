...
static int TempArry[5][2] = {{1000,570},{600,420},{450,100},{130,30},{60,-200}};
static int asus_battery_update_status_no_mutex(int percentage)
{
        int status;
        static int temperature,old_temperature;
	...
	uint8_t ctrldata;
        ...
        if (cable_status == USB_ADAPTER || cable_status == USB_PC) {
                ...
                if (fac_charge_disable != 0 || (percentage >= 55 && tmp_batt_info.eng_charging_limit)) {
                        BAT_DBG("in fac mode and capasity > 60% or write stop charge 1\n");
                        smb347_charging_toggle(false);
			...
			if(tmp_batt_info.batt_volt < 4250) 
				smb347_set_recharge(false);
			else
				smb347_set_recharge(true);	
			...
			temperature = asus_battery_update_temp_no_mutex();
					...
					if(PROJ_ID_A500CG == Read_PROJ_ID()) {
						if((temperature-old_temperature) > 0){
							if(temperature > TempArry[index][0])
								index-= 1;
						}else{
							if(temperature < TempArry[index][1])
								index+= 1;
						}
						printk("temp:%d,oldtemp:%d,index:%d\n",temperature,old_temperature,index);
						...
						switch (index)
						{
							...
							case 1:
								temp_status_502 = 0;
								if(tmp_batt_info.batt_volt>=4110){
									smb347_charging_toggle(false);
									status = POWER_SUPPLY_STATUS_DISCHARGING;
								}else
									smb347_charging_toggle(true);
...
final:
	...
	if(entry_mode == 4) {
		intel_scu_ipc_ioread8(0x195, &ctrldata);
		ctrldata = (ctrldata&0xfe);
		intel_scu_ipc_iowrite8(0x195, ctrldata);
