...
#define OTG_CURRENT_LIMIT_250mA	0
...
#define CFG_434V_SMB358				BIT(1)|BIT(3)|BIT(5)
...
static int otg(int toggle)
{
	...
	if (toggle) {
		if( ((hw_id_flag==0)&&(project_id_flag==0))||((hw_id_flag==2)&&(project_id_flag==0)) ) {
			/* 1.Disable OTG */
			...
			//switch frequency to 1.5MHZ
			ret = smb347_masked_write(smb347_dev->client, THERM_CTRL_A_REG,
				BIT(7), BIT(7));
			if (ret) {
				CHR_err("Failed to set OTG frequency to 1.5MHZ. ret=%d\n", ret);
				return ret;
			}
			...
			/* 4.Set OTG current limit to 500mA & UVLO 2.7V*/
			// Refer to SMB347 Application Note 72 to solve serious problems
			ret = smb347_masked_write(smb347_dev->client, OTG_TLIM_THERM_CNTRL_REG,
				BIT(0)|BIT(1)|BIT(2)|BIT(3), BIT(2));
			if (ret) {
				CHR_err("Failed to set OTG current limit 500mA. ret=%d\n", ret);
				return ret;
			}
		...
		if( (hw_id_flag>=1)||(project_id_flag==1) ) {
			/* 1.Set OTG current limit to 250mA & UVLO 2.7V*/
			// Refer to SMB347 Application Note 72 to solve serious problems
			ret = smb347_masked_write(smb347_dev->client, OTG_TLIM_THERM_CNTRL_REG,
				BIT(0)|BIT(1)|BIT(2)|BIT(3), 0);
			if (ret) {
			...
			/*power on reset*/
			ret = smb347_masked_write(smb347_dev->client, CMD_B_REG,
				BIT(7), BIT(7));
			if (ret) {
				CHR_err("Failed to set power on reset ret=%d\n", ret);
			return ret;
			}
  ...
  ret &= ~(BIT(0)|BIT(1));
	if (on) {
		...
		if(project_id_flag==2)
			ret &= ~(BIT(2)|BIT(3));
		if((project_id_flag==0)&&(hw_id_flag==2))
			ret &= ~(BIT(2)|BIT(3));//add 451 soft cold temp limit  no response
			
	} else {
		// Float Voltage Compensation
		ret |= BIT(1);
		if((project_id_flag==2)||((project_id_flag==0)&&(hw_id_flag==2))) {
...
int smb347_set_recharge(bool on)
{
	 int ret = 0;
	/* Config recharge register */
	ret = smb347_read(smb347_dev, CFG_CURRENT_LIMIT);
	ret &=0xF3;//recharge 50mV
	/* recharge inhibit = 200mV, 01h[3:2] = 10*/
	if(on)
		ret |= (BIT(3));

	printk("write recharge voltage = 0x%02x\n", ret);
	smb347_write(smb347_dev, CFG_CURRENT_LIMIT, ret);
}
	 ...
	 if((project_id_flag==0)||(project_id_flag==2)) {
		 if( hw_id_flag==0 )
			...
			if((project_id_flag==2)&&((temp_status_502==0)||(temp_status_502==1))) {
				//in A502CG, small current when in low temp 450mA
				ret &= ~(BIT(7)|BIT(6));
				ret |= (BIT(5));
			}else {//900mA
				ret &= ~(BIT(7));
...
extern unsigned char charge_suspend_en;
...
static struct proc_dir_entry *charging_disable_proc_file;
char charging_disable =0;// 1 means disable , 0 means enable
char charging_dis=0xff;
...
static ssize_t charging_disable_proc_write(struct file *filp, const char __user *buffer, size_t count, loff_t *ppos)
{
	ssize_t ret = 0;
	u8 reg;
	
	ret = copy_from_user(&charging_dis,buffer,sizeof(charging_disable));
	//0 's ASSIC is 48 , and we only receive 0 or 1 here 
	if((charging_dis!=48)&&(charging_dis!=49))
		return -EFAULT;
	charging_disable = charging_dis-48;
	if(charging_disable){
		smb347_read_reg(smb347_dev->client, 0x30, &reg, 0);
		reg |= 0x04;
		smb347_write_reg(smb347_dev->client, 0x30, reg);
		
		smb347_read_reg(smb347_dev->client, 0x30, &reg, 0);
		printk("charging_disable_proc_write disable %x\n",reg);
	}
	else{
			smb347_read_reg(smb347_dev->client, 0x30, &reg, 0);
			reg &= 0xFB;
			...
			smb347_read_reg(smb347_dev->client, 0x30, &reg, 0);
			printk("charging_disable_proc_write enable %x\n",reg);
		}
	return 1;
}

static struct file_operations charging_disable_proc_ops = {
    .write = charging_disable_proc_write,
};

static void create_charging_disable_proc_file(void)
{
    charging_disable_proc_file = proc_create("Charge_Disable", 0666,NULL, &charging_disable_proc_ops);
    if(!charging_disable_proc_file)
		printk("create charging_disable_proc_file fail\n");
			...
			const struct i2c_device_id *id)
{
  ...
  cable_status_notify2( NULL, query_cable_status(), dev);
	...
	if(charge_suspend_en==1 && smb347_get_charging_status() == POWER_SUPPLY_STATUS_CHARGING ) {
		if (!wake_lock_active(&wakelock_cable)) {
	...
	not_ready_flag = 0;
	create_charging_disable_proc_file();
...
static int smb347_suspend(struct device *dev)
{
    ...
    if((hw_id_flag==2)&&(project_id_flag==0)) 
		smb347_control_JEITA(false);
