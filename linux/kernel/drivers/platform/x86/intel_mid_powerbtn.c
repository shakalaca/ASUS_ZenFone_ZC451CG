...
extern unsigned char disable_power_key;
extern unsigned char enter_keypad_test_mode;
unsigned char power_key_value;
void set_power_key_value(unsigned char value)
{
	power_key_value = value;
}
#endif
...
static irqreturn_t mid_pb_isr(int irq, void *dev_id)
{
...
#ifdef CONFIG_FACTORY_ITEMS
	if(disable_power_key == 0 && enter_keypad_test_mode == 0)
	{
	...
	{
		__clear_bit(KEY_POWER, priv->input->keybit);
		__set_bit(KEY_B, priv->input->keybit);
		input_event(priv->input, EV_KEY, KEY_B, !(pbstat & priv->pb_level));
		input_sync(priv->input);
		__clear_bit(KEY_B, priv->input->keybit);
