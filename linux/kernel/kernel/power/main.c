...
unsigned char charge_suspend_en=1;
static ssize_t charge_suspend_en_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	if(charge_suspend_en==1) {
		strcpy(buf,"1\n");
		return sizeof("1\n");
	}
	else if(charge_suspend_en==0) {
		strcpy(buf,"0\n");
		return sizeof("0\n");
	}
	return -EINVAL;
}
static ssize_t charge_suspend_en_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t n)
{
	if(strcmp(buf,"1\n")==0) {
		charge_suspend_en=1;
		return n;
